...
from app.utils.logging_utils import (
    log_request_start, log_request_end, log_business_milestone,
    log_db_operation, log_error_with_context
)
import logging
import json
import time
...
async def upload_knowledge(
    ...
    try:
        log_business_milestone(
            logger, "document_ingestion_start",
            user_id=request.user_id,
            details={"filename": request.filename, "content_length": len(request.content)}
        )
        ...
        log_business_milestone(
            logger, "document_ingestion_complete",
            user_id=request.user_id,
            details={
                "filename": request.filename,
        ...
        log_error_with_context(
            logger, e, "knowledge_upload_error",
            context={
                "user_id": request.user_id,
                "filename": request.filename,
...
async def list_knowledge(user_id: str):
    ...
    log_request_end(
        logger, "GET", f"/knowledge/{user_id}",
        status_code=200,
        duration_ms=(time.time() - start_time) * 1000,
        user_id=user_id
    )
