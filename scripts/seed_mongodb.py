...
def insert_batch(
    collection, documents: List[Dict], batch_size: int = 100
) -> List[Dict]:
    """
...
def generate_users(count: int) -> List[Dict]:
    ...
    all_subcategories = customer_subcategories + [None] * (
        customer_care_count + area_manager_count
    )
...
def generate_orders(
    count: int, users: List[Dict], restaurants: List[Dict], zones: List[Dict]
) -> List[Dict]:
    ...
    customer_users = [
        u for u in users if u.get("persona") == UserPersona.CUSTOMER.value
    ]
    ...
    for user_id, order_count in user_order_counts.items():
        for _ in range(order_count):
            ...
            item_name = (
                ...
                + " "
                + random.choice(
                    ["Burger", "Pizza", "Curry", "Biryani", "Noodles", "Soup", "Salad"]
                )
            )
            ...
            if random.random() < 0.15:  # 15% have refunds
                refund_status = random.choice(
                    [RefundStatus.PENDING, RefundStatus.ISSUED, RefundStatus.COMPLETED]
                )
                ...
                refund = {
                    ...
                    "issued_at": (
                        order_time + timedelta(hours=random.randint(1, 24))
            ...
            if order_status != OrderStatus.CANCELLED:
                ...
                if order_status in [OrderStatus.IN_TRANSIT, OrderStatus.DELIVERED]:
                    pickup_time = confirm_time + timedelta(
...
def generate_support_tickets(
    ...
    ticket_types = [TicketType.COMPLAINT] * int(count * 0.6) + [TicketType.GENERAL] * (
        count - int(count * 0.6)
    )
    ...
    for i in range(order_scoped_count):
        ...
        issue_type = random.choices(
            [it[0] for it in issue_types], weights=[it[1] for it in issue_types]
        )[0]
...
def update_ticket_relationships(db, tickets: List[Dict], orders: List[Dict]):
    ...
    order_scoped_tickets = [
        t for t in tickets if t.get("scope") == TicketScope.ORDER.value
    ]
    if order_scoped_tickets:
        ...
        for ticket in tickets_with_related:
            if orders:
                ...
                if other_orders:
                    related_orders = random.sample(
                        other_orders, k=min(random.randint(1, 3), len(other_orders))
                    )
                    collection.update_one(
                        {"_id": ticket["_id"]},
                        {
                            "$set": {
                                "related_orders": [o["_id"] for o in related_orders]
                            }
                        },
                    )
                    updated_count += 1
    ...
    if len(tickets) > 1:
        tickets_with_related_tickets = random.sample(
            tickets, k=min(int(len(tickets) * 0.2), len(tickets))
        )
        ...
        for ticket in tickets_with_related_tickets:
            ...
            if ticket_index > 0:
                ...
                if previous_tickets:
                    related_tickets = random.sample(
                        previous_tickets,
                        k=min(random.randint(1, 2), len(previous_tickets)),
                    )
                    collection.update_one(
                        {"_id": ticket["_id"]},
                        {
                            "$set": {
                                "related_tickets": [t["_id"] for t in related_tickets]
                            }
                        },
                    )
                    updated_count += 1
    ...
    for zone in zones:
        ...
        while current_time <= now:
            ...
            metric = {
                ...
                + random.randint(0, 10),  # Add some variance
...
def generate_restaurant_metrics_history(
    restaurants: List[Dict], orders: List[Dict]
) -> List[Dict]:
    ...
    for restaurant in restaurants:
        ...
        while current_time <= now:
            ...
            hour_orders = [
                o
                for o in restaurant_order_list
                if hour_start
                <= datetime.fromisoformat(o["created_at"].replace("Z", "+00:00"))
                < hour_end
            ]
...
def update_order_ids(db, orders: List[Dict]):
    ...
    for order in orders:
        order_id = order["_id"]
        collection.update_one({"_id": order_id}, {"$set": {"order_id": order_id}})
        updated_count += 1
...
def update_ticket_ids(db, tickets: List[Dict]):
    ...
    for ticket in tickets:
        ...
        collection.update_one({"_id": ticket_id}, {"$set": {"ticket_id": ticket_id}})
        updated_count += 1
...
def update_user_stats(db, users: List[Dict], orders: List[Dict]):
    ...
    for user in users:
        ...
        refund_count = sum(
            ...
            if o.get("refund_status") != RefundStatus.NONE.value
        )
        ...
        collection.update_one({"_id": user_id}, {"$set": update_doc})
        updated_count += 1
...
def implement_test_scenarios(
    ...
    # 1. High-risk customer with multiple refunds
    high_risk_users = [
        u for u in users if u.get("sub_category") == CustomerSubCategory.HIGH_RISK.value
    ]
    if high_risk_users and orders:
        ...
        refund_orders = random.sample(
            user_orders, k=min(random.randint(3, 5), len(user_orders))
        )
        for order in refund_orders:
            if order.get("refund_status") == RefundStatus.NONE.value:
                order["refund"] = {
                    "amount": round(
                        random.uniform(0.1, float(order["total_amount"])), 2
                    ),
    ...
    critical_tickets = [
        t for t in tickets if t.get("severity") == TicketSeverity.CRITICAL.value
    ]
        ...
        for _ in range(2 - len(critical_tickets)):
            ...
            ticket = {
                ...
                "issue_type": random.choice(
                    [IssueType.QUALITY_SAFETY, IssueType.OPERATIONS]
                ).value,
                "subtype": (
                    {"quality_safety": [QualitySafetySubtype.FOOD_SAFETY.value]}
                ...
                "affected_zones": (
                    [random.choice(zones)["_id"]] if zones and not order else []
                ),
                "affected_city": (
                    random.choice(zones)["city"] if zones and not order else None
                ),
    ...
    platinum_users = [
        u for u in users if u.get("sub_category") == CustomerSubCategory.PLATINUM.value
    ]
    ...
    paused_restaurants = [
        r for r in restaurants if r.get("status") == RestaurantStatus.PAUSED.value
    ]
...
def create_indexes(db):
    ...
    db.support_tickets.create_index(
        [("restaurant_id", 1), ("timestamp", -1)], sparse=True
    )
    ...
    db.zone_metrics_history.create_index(
        [("timestamp", -1)], expireAfterSeconds=604800
    )  # 7-day TTL

    # Restaurant metrics history indexes
    db.restaurant_metrics_history.create_index(
...
def main():
    ...
    parser.add_argument(
        "--force", action="store_true", help="Delete existing data before seeding"
    )
    parser.add_argument(
        "--create-indexes", action="store_true", help="Create MongoDB indexes"
    )
    ...
    try:
        ...
        if not args.force:
            ...
            if has_data:
                print(
                    "WARNING: Collections already contain data. Use --force to delete and recreate."
                )
                return
        ...
        tickets_docs = generate_support_tickets(
            ticket_count, users, orders, zones, restaurants
        )
